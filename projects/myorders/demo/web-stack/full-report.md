...
|------|-----------|---------|--------|---------|
| Reverse Proxy | Nginx | 1.21.5 | :80, :8000-8002 | SSL termination (externo), SPA serving, API proxy |
| API Backend | PHP-FPM | 8.1.23 | :9000 | FastCGI — procesa Symfony |
| Framework | Symfony | 6.1 | — | Monorepo con 8 sub-apps |
| Frontend | Webpack Encore | Bootstrap 5 + Stimulus | — | Assets compilados |
| Base de datos | MariaDB | 10.11.9 | :3306 | Datos de pedidos, usuarios |
| Backend externo | SAP ERP | — | SOAP/OData | Pedidos, precios, stock |
...
> ℹ️ **Nota**: El vhost de producción (`newmyorders.werfen.com`) incluye headers de seguridad adicionales: `Cache-Control`, `Content-Security-Policy`. El vhost de demo **no** los tiene.
...
├── assets/                  ← Frontend source (JS/SCSS por app)
├── deployment/              ← CI/CD configs (Jenkinsfile, Azure Pipelines)
├── tests/                   ← PHPUnit tests
├── playwright-test/         ← E2E tests (Playwright)
...
> ⚠️ **Error en build/app.js — 404**: Los admin panels (demo y prod) no encuentran `/build/app.js`. Los assets de Webpack no se han compilado o desplegado correctamente para ordersbackoffice.
...
> ⚠️ **Nmap scan detectado**: El 25/04/2026 a las 06:49 se detectó un escaneo Nmap desde `212.163.185.1` contra `caseportal.werfen.com`. Se probaron múltiples rutas (/owa/, /webui, /HNAP1, /sdk, /versa/login, etc.).
