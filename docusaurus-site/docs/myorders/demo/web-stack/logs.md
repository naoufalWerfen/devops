---
sidebar_label: "Mapa de Logs"
title: "MyOrders DEMO — Mapa de Logs"
---
...
:::warning Monolog → stderr
En producción, Symfony Monolog está configurado con `path: php://stderr`, lo que envía **todos los logs de aplicación al error.log de Nginx** a través de FastCGI. No hay archivos de log Symfony separados.
:::
...
:::info Logs vacíos en entorno demo
Los directorios `var/log/` de todas las aplicaciones **están vacíos**, lo que confirma que el entorno demo ejecuta en modo `prod` de Symfony y los logs van a stderr (capturados por Nginx):
...
Los siguientes servicios tienen configuración de rotación detectada:
...
:::caution Filtrado necesario
Al no haber separación de logs por vhost, es necesario filtrar por dominio (`server:`) en el error.log para aislar problemas de una aplicación específica.
:::
